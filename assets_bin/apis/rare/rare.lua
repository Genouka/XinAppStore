...
local CACHE_APPLIST="rare_cache_applist"
local CACHE_BANNER="rare_cache_banner"
local CACHE_CATEGORY="rare_cache_category"
local CACHE_CONFIG_TIME="rare_cache_config"
--[[
...
function edang.applist(callback)
  edang_inner.pb_apidata(callback,CACHE_APPLIST,"https://pass.genouka.run.place/store/api.php",
  function(info,data)
    --print(data)
    local t=json.decode(data)
    for i=1,#t do
      t[i].name=t[i].title
      t[i].author=t[i].autor
      t[i].description=t[i].content
    end
    callback.onsuccess(info,t)
  end)
end
